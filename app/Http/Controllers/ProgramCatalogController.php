...
class ProgramCatalogController extends Controller
{
    ...
    {
        $catalog = Cache::remember('public.programs.safe.'.sha1($request->getQueryString() ?? 'index'), now()->addMinutes(5), function () use ($request): array {
        ...
        return Inertia::render('software/index', ['catalogType' => 'programs', ...$catalog, 'filters' => $request->only(['q', 'category', 'platform'])]);
    }
